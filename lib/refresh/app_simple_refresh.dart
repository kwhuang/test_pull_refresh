...
import 'need_clear_photo_cache.dart';

typedef Widget BuildListItemCallBack<A>(BuildContext context, A data, int index);

///
/// 上下拉刷新-适用于普通列表，Sliver列表，NesSliver列表，瀑布流，Gride表格
/// 下拉刷新使用flutter sdk自带，
/// 加载更多使用第三方插件，文档：https://github.com/fluttercandies/loading_more_list/blob/master/README-ZH.md
///
/// 瀑布流，表格，相关回调
///
class AppSimpleRefresh<T> extends StatefulWidget {
  @override
  AppSimpleRefreshState<T> createState() => AppSimpleRefreshState<T>();
  ...
  /// 是否需要清理列表中的图片缓存，default=false，true时需要同时满足以下两个条件才有效
  /// 1：数据类需要 mixin NeedClearCache.dart
  /// 2：图片组件需要使用getAppNetWorkImage
...
class AppSimpleRefreshState<T> extends State<AppSimpleRefresh<T>> {
  //数据源
  late CustomLoadingMoreBase<T> _listSource;
  ...
  void initState() {
    ...
    _listSource = CustomLoadingMoreBase(
    ...
    _loadingIndicator = CustomLoadingMoreIndicator(
      ...
      loadingMoreBusyingWidgt: widget.loadingMoreBusyingWidgt,
      fullScreenBusyingWidgt: widget.fullScreenBusyingWidgt,
      errorWidgt: widget.errorWidgt,
      fullScreenErrorWidgt: widget.fullScreenErrorWidgt,
      noMoreLoadWidgt: widget.noMoreLoadWidgt,
      emptyWidgt: widget.emptyWidgt,
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isNes == true) {
      return getAppDefaultRefreshIndicator4Nested(
        key: _refreshKey,
    ...
    return getAppDefaultRefreshIndicator(
      ...
      child: loadMoreList,
      isSliver:widget.isSliver,
      haveRefresh: widget.haveRefresh
    );
  }
  ...
  /// 更新某一个指定项
  void replaceItemByIndex(int index, T newItem) {
    if (index == -1 || _listSource == null || index == null || index >= _listSource.length) {
      return;
    }
    _listSource[index] = newItem;
  ...
  ScrollPhysics get _defaultPhysics => Platform.isIOS ? BouncingScrollPhysics() : ClampingScrollPhysics();
  ...
  Widget _buildJustLoadMoreList() {
    return LoadingMoreList<T>(
      ListConfig<T>(
        ...
        // collectGarbage: widget.needClearPhotoCache == true ? _collectGarbage : null,
      ),
    );
  }
  ...
  Widget _buildJustLoadMoreList4Sliver() {
    ...
    var _slivers = widget.haveRefresh == false ? <Widget>[] : <Widget>[PullToRefreshContainer(
    ...
    //添加头部，如果有的话
    if (widget.headSlivers.length > 0) {
    ...
    //添加底部
    if (widget.bottomSlivers.length > 0) {
  ...
  void _clearItemMemory(T item) {
    if (item is NeedClearPhotoCache<T>) {
      var photoPaths = item.getClearPhotoPaths(item);
      // clearPhotosMemoryCache(httpPhotoUrls: photoPaths);
    }
  }
}
